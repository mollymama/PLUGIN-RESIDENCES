...
class Rhino3dmLoader extends Loader {
	...
	_createGeometry( data ) {
		...
		const instanceDefinitionsById = new Map();
		const instanceDefinitionObjectsById = new Map();
		const instanceReferencesById = new Map();
		const instanceDefinitionChildIds = new Set();

		for ( let i = 0; i < instanceDefinitions.length; i ++ ) {

			const iDef = instanceDefinitions[ i ];
			instanceDefinitionsById.set( iDef.attributes.id, iDef );

			for ( let j = 0; j < iDef.attributes.objectIds.length; j ++ ) {

				instanceDefinitionChildIds.add( iDef.attributes.objectIds[ j ] );

			}

		}

		for ( let i = 0; i < instanceDefinitionObjects.length; i ++ ) {

			const iDefObject = instanceDefinitionObjects[ i ];
			instanceDefinitionObjectsById.set( iDefObject.userData.attributes.id, iDefObject );

		}

		for ( let i = 0; i < instanceReferences.length; i ++ ) {

			const iRef = instanceReferences[ i ];
			instanceReferencesById.set( iRef.attributes.id, iRef );

		}

		const createInstanceReferenceObject = ( iRef, ancestry = [] ) => {

			const parentIdefId = iRef.geometry.parentIdefId;
			const instanceObject = new Object3D();
			const xf = iRef.geometry.xform.array;
			const matrix = new Matrix4();

			matrix.set( ...xf );
			instanceObject.applyMatrix4( matrix );

			instanceObject.userData[ 'attributes' ] = iRef.attributes;
			instanceObject.userData[ 'objectType' ] = 'InstanceReference';
			instanceObject.userData[ 'instanceDefinitionId' ] = parentIdefId;
			instanceObject.userData[ 'parentIdefId' ] = parentIdefId;
			instanceObject.userData[ 'instanceTransform' ] = xf.slice();
			instanceObject.userData[ 'instanceTransformFixed' ] = true;

			if ( iRef.attributes.name ) {

				instanceObject.name = iRef.attributes.name;

			}

			if ( ancestry.includes( parentIdefId ) ) {

				console.warn( `THREE.3DMLoader: Circular InstanceDefinition reference skipped for ${parentIdefId}.` );
				return instanceObject;

			}

			const definitionObject = createInstanceDefinitionObject( parentIdefId, ancestry.concat( parentIdefId ) );

			for ( let i = 0; i < definitionObject.children.length; i ++ ) {

				instanceObject.add( definitionObject.children[ i ].clone( true ) );
		...
		const createInstanceDefinitionObject = ( iDefId, ancestry = [] ) => {

			const definitionObject = new Object3D();
			const iDef = instanceDefinitionsById.get( iDefId );
			...
			definitionObject.userData[ 'attributes' ] = iDef.attributes;
			definitionObject.userData[ 'objectType' ] = 'InstanceDefinition';

			if ( iDef.attributes.name ) {

				definitionObject.name = iDef.attributes.name;
			...
			for ( let j = 0; j < iDef.attributes.objectIds.length; j ++ ) {
				...
				const definitionChild = instanceDefinitionObjectsById.get( objId );
				const nestedReference = instanceReferencesById.get( objId );

				if ( definitionChild ) {

					definitionObject.add( definitionChild.clone( true ) );

				} else if ( nestedReference ) {

					definitionObject.add( createInstanceReferenceObject( nestedReference, ancestry ) );
		...
		for ( let i = 0; i < instanceReferences.length; i ++ ) {
			...
			if ( instanceDefinitionChildIds.has( iRef.attributes.id ) ) {

				continue;

			}

			object.add( createInstanceReferenceObject( iRef ) );
